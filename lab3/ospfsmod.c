...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
        ...
        int od_type;
        if (entry_oi->oi_ftype == OSPFS_FTYPE_REG) od_type = DT_REG;
        else if (entry_oi->oi_ftype == OSPFS_FTYPE_DIR) od_type = DT_DIR;
        else od_type = DT_LNK;

		ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos, od->od_ino, od_type);
		if (ok_so_far >= 0)
			f_pos++;
        else
            break;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		data = (char*)ospfs_block(blockno) + offset;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
    ...
    uint32_t skipped = *f_pos;
    uint32_t remaining = oi->oi_size - skipped;
    if (count > remaining) {
        int status = change_size(oi, skipped + count);
        if (status)
            return status;
    }
